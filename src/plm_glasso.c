...
int plm_glasso(model_t *m) {
  ...
  while (t < m->glasso_iter) {
    ...
    int lbfgs_ret = lbfgs(nvar, m->x, &fx, evaluate_plm_glasso,
                          progress_plm_glasso, m, &param);
    ...
    printf(
        "glasso iter = %3d rho= %.3f lambda_rho= %.3f eps= %.6f ret_eps= %.6f "
        "lbfgs_ret= %2d rho= %.4f x_norm= %.3f z_norm=%.3f u_norm= %.3f\n",
        t + 1, m->glasso_rho, lambda_rho, eps, eps_ret, lbfgs_ret,
        m->glasso_rho, x_norm, z_norm, u_norm);
...
lbfgsfloatval_t evaluate_plm_glasso(void *instance, const lbfgsfloatval_t *x,
  ...
  int ncol = model->ncol;
  double *w = model->w;
  double neff = model->neff;

  int threads_num = model->threads_num;
  int nsingle = ncol * ALPHA;
  ...
  double **gradient_all = model->gradient_all;
...
#pragma omp parallel for
  ...
  // add regularization
  lbfgsfloatval_t lambda_single = model->lambda_single * neff;
  lbfgsfloatval_t lambda_pair = model->lambda_pair * neff;
  ...
  // glasso
  double *u = model->glasso_u;
  double *z = model->glasso_z;
  double rho = model->glasso_rho;
