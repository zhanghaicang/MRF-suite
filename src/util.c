...
void cal_apc(double *m, double* apc, int ncol){
    ...
    for(int i = 0; i < ncol; i++){
        for(int j = i+1; j < ncol; j++){
            apc[i*ncol+j] = apc[j*ncol+i] = m[i*ncol+j] - mean[i] * mean[j] / sum;
        }
    }
    free(mean);
}
...
void cal_acc(double*mat, double*dis, int ncol, double*acc){
    ...
    score_item* scores = (score_item*) malloc(sizeof(score_item) * num);
    ...
    qsort(scores, num, sizeof(score_item), compare);
    for(int i = 0; i < 4; i++){
...
int compare (const void * a, const void * b){
    score_item* a1 = (score_item*) a;
    score_item* b1 = (score_item*) b;
